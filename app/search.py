...
class DocumentStore:
    ...
    def replace_all(self, docs: list[dict[str, Any]]) -> None:
        with self.connect() as conn:
            ...
            conn.executemany("insert into documents(id,title,category,section,text,url,tags,updated) values(?,?,?,?,?,?,?,?)", [(d["id"], d["title"], d["category"], d.get("section", "General"), d["text"], d.get("url", ""), "|".join(d.get("tags", [])), time.time()) for d in docs])

    def upsert(self, doc: dict[str, Any]) -> None:
        with self.connect() as conn:
            conn.execute("insert into documents(id,title,category,section,text,url,tags,updated) values(?,?,?,?,?,?,?,?) on conflict(id) do update set title=excluded.title,category=excluded.category,section=excluded.section,text=excluded.text,url=excluded.url,tags=excluded.tags,updated=excluded.updated", (doc["id"], doc["title"], doc["category"], doc.get("section", "General"), doc["text"], doc.get("url", ""), "|".join(doc.get("tags", [])), time.time()))
    ...
    def get(self, doc_id: str) -> dict[str, Any] | None:
        ...
        if not row:
            return None
        data = dict(row); data["tags"] = [x for x in data["tags"].split("|") if x]
        return data
    ...
    def popular_queries(self, limit: int = 8) -> list[dict[str, Any]]:
        with self.connect() as conn:
            rows = conn.execute("select query,count(*) as count,avg(latency_ms) as avg_latency_ms,max(created) as latest from queries group by query order by count desc,latest desc limit ?", (limit,)).fetchall()
...
class SearchEngine:
    def __init__(self, docs: list[dict[str, Any]] | None = None, store: DocumentStore | None = None):
        ...
        if docs is not None: self.store.replace_all(docs)
        elif self.store.count() == 0: self.store.replace_all(DEFAULT_DOCS)
        self.indexed_at = 0.0; self.rebuild()

    def rebuild(self) -> None:
        self.docs = self.store.all()
        if not self.docs:
            self.tokens=[]; self.bm25=None; self.word_vectorizer=None; self.word_matrix=None; self.char_vectorizer=None; self.char_matrix=None; self.semantic_matrix=None; self.semantic_transform=None; self.indexed_at=time.time(); return
        weighted = [f"{d['title']} {d['title']} {d['section']} {' '.join(d['tags'])} {d['text']}" for d in self.docs]
        self.tokens=[tokenize(text) for text in weighted]; self.bm25=BM25Okapi(self.tokens)
        self.word_vectorizer=TfidfVectorizer(ngram_range=(1,2),sublinear_tf=True,min_df=1); self.word_matrix=self.word_vectorizer.fit_transform(weighted)
        self.char_vectorizer=TfidfVectorizer(analyzer="char_wb",ngram_range=(3,5),sublinear_tf=True,min_df=1,max_features=5000); self.char_matrix=self.char_vectorizer.fit_transform(weighted)
        components=min(64,max(2,min(self.word_matrix.shape)-1))
        if self.word_matrix.shape[0]>=3 and self.word_matrix.shape[1]>=3:
            svd=TruncatedSVD(n_components=components,random_state=7); normalizer=Normalizer(copy=False); self.semantic_matrix=normalizer.fit_transform(svd.fit_transform(self.word_matrix)); self.semantic_transform=(svd,normalizer)
        else: self.semantic_transform=None; self.semantic_matrix=self.word_matrix.toarray()
        self.indexed_at=time.time()

    def add_document(self, doc: dict[str, Any]) -> None:
        self.store.upsert(doc); self.rebuild()

    def _orders(self, query: str):
        expanded=expand_query(query); bm=np.asarray(self.bm25.get_scores(tokenize(expanded)),dtype=float); word=cosine_similarity(self.word_vectorizer.transform([expanded]),self.word_matrix)[0]; char=cosine_similarity(self.char_vectorizer.transform([expanded]),self.char_matrix)[0]
        if self.semantic_transform:
            svd,norm=self.semantic_transform; q=norm.transform(svd.transform(self.word_vectorizer.transform([expanded]))); semantic=cosine_similarity(q,self.semantic_matrix)[0]
        else: semantic=word
        semantic=.7*semantic+.3*char
        return {"bm25":(list(np.argsort(bm)[::-1]),bm),"semantic":(list(np.argsort(semantic)[::-1]),semantic),"tfidf":(list(np.argsort(word)[::-1]),word)}

    def search(self, query: str, category: str | None=None, section: str | None=None, limit: int=10, mode: str="hybrid", explain: bool=False):
        started=time.perf_counter(); query=query.strip()
        if not query or not self.docs: return {"query":query,"mode":mode,"results":[],"total":0,"facets":{},"latency_ms":0.0}
        orders=self._orders(query)
        if mode=="bm25": order=orders["bm25"][0]; fusion={i:float(orders["bm25"][1][i]) for i in order}
        elif mode in {"semantic","dense"}: order=orders["semantic"][0]; fusion={i:float(orders["semantic"][1][i]) for i in order}
        else: order,fusion=rrf(orders["bm25"][0],orders["semantic"][0],orders["tfidf"][0])
        qterms=set(tokenize(query)); rows=[]
        for idx in order:
            d=self.docs[idx]
            if category and d["category"]!=category: continue
            if section and d["section"]!=section: continue
            title_overlap=len(qterms & set(tokenize(d["title"])))/max(1,len(qterms)); base=float(fusion.get(idx,0)); score=base+(.02*title_overlap if mode=="hybrid" else 0)
            row={"id":d["id"],"title":d["title"],"category":d["category"],"section":d["section"],"url":d.get("url",""),"tags":d.get("tags",[]),"snippet":snippet(d["text"],query),"score":round(score,6)}
            if explain: row["explain"]={"bm25":round(float(orders["bm25"][1][idx]),6),"semantic":round(float(orders["semantic"][1][idx]),6),"tfidf":round(float(orders["tfidf"][1][idx]),6),"title_overlap":round(title_overlap,4),"fusion":round(base,6)}
            rows.append(row)
        rows.sort(key=lambda r:r["score"],reverse=True); rows=rows[:max(1,min(limit,50))]; latency=(time.perf_counter()-started)*1000; self.store.log_query(query,mode,len(rows),latency)
        return {"query":query,"mode":mode,"results":rows,"total":len(rows),"facets":{"categories":dict(sorted(Counter(d["category"] for d in self.docs).items())),"sections":dict(sorted(Counter(d["section"] for d in self.docs).items()))},"latency_ms":round(latency,3)}

    def suggest(self,prefix:str,limit:int=8):
        p=prefix.strip().lower(); values=[]
        for d in self.docs: values.extend([d["title"],d["category"],d["section"],*d.get("tags",[])])
        return [v for v,_ in Counter(v for v in values if p in v.lower()).most_common(limit)]

    def stats(self):
        return SearchStats(len(self.docs),len({d["category"] for d in self.docs}),len(self.word_vectorizer.vocabulary_) if self.word_vectorizer else 0,self.store.query_count(),self.indexed_at)


def evaluate(engine: SearchEngine):
    labels={"persist container data":{"docker-vol"},"validate API JSON":{"fastapi-validation"},"rolling pod updates":{"k8s-deploy"},"running total SQL":{"sql-window"},"combine lexical semantic rankings":{"ir-rrf"},"self attention NLP":{"ml-transformer"}}
    output={}
    for mode in ["bm25","semantic","hybrid"]:
        recalls=[]; rr=[]
        for query,relevant in labels.items():
            ids=[r["id"] for r in engine.search(query,limit=3,mode=mode)["results"]]; recalls.append(len(set(ids)&relevant)/len(relevant)); rank=next((i+1 for i,x in enumerate(ids) if x in relevant),None); rr.append(1/rank if rank else 0)
        output[mode]={"recall@3":round(float(np.mean(recalls)),4),"mrr@3":round(float(np.mean(rr)),4)}
    return output
